...
class BabyRepository {
    ...
    suspend fun deleteBaby(babyId: String) {
        listOf("baby_logs", "invite_codes", "baby_members", "babies").forEach { table ->
            val col = if (table == "babies") "id" else "baby_id"
            client.from(table).delete { filter { eq(col, babyId) } }
        }
    }

    suspend fun generateInviteCode(babyId: String, userId: String): String {
        val code = (1..6).map { ('A'..'Z').random() }.joinToString("")
        val expires = java.text.SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss+09:00", java.util.Locale.getDefault())
